### DATASETS TO USE ###
version_to_use <- '21q1'
version_url <- "https://figshare.com/articles/dataset/public_21q1/13681534"

# https://figshare.com/ndownloader/files/26261569 https://figshare.com/ndownloader/files/26261569
sample_info_url <- "https://figshare.com/ndownloader/files/26261569"
# https://figshare.com/ndownloader/files/26261293 https://figshare.com/ndownloader/files/26261293
gene_effect_url <- "https://figshare.com/ndownloader/files/26261293"
# 
gene_expression_url <- "https://figshare.com/ndownloader/files/26261500"
...
### FORMAT SAMPLE INFO ###
if(!dir.exists('data_21Q1')) { dir.create('data_21Q1') }

if(!file.exists("data_21Q1/sample_info.csv")) # If file does not exist, download from FigShare. Otherwise load local file.
{
  sample_info <- fread(sample_info_url)
  fwrite(sample_info, "data_21Q1/sample_info.csv")
} else
{
  sample_info <- fread("data_21Q1/sample_info.csv")
}

# Fix annotations.
mf <- sample_info %>%
  filter(!is.na(DepMap_ID) & DepMap_ID != '') %>%
  dplyr::select(DepMap_ID, CCLE_name=CCLE_Name, Type=primary_disease, T1=lineage, T2=lineage_subtype, T3=lineage_sub_subtype, age) %>%
...
load_gene_effect <- function(){
  if(file.exists("data_21Q1/gene_effect.csv.zip") & !file.exists("data_21Q1/gene_effect.csv"))
  {
    unzip("data_21Q1/gene_effect.csv.zip", exdir = "data_21Q1/")
  }
  
  if(!file.exists("data_21Q1/gene_effect.csv")) # If file does not exist, download from FigShare. Otherwise load local file.
  {
    gene_effect <- fread(gene_effect_url)
    fwrite(gene_effect, "data_21Q1/gene_effect.csv")
  } else
  {
    gene_effect <- fread("data_21Q1/gene_effect.csv")
  }
  # Reformat as a matrix.
  gene_effect %<>% dplyr::filter(DepMap_ID %in% mf$DepMap_ID)
  rownames_temp <- gene_effect$DepMap_ID
